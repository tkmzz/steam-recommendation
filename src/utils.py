...
#Function to capture implicit reviews
...
#Generic class for recommendation
class ItemBasedRecomemnder:
    ...
    def recommend(self, target_item, max_recommendation=None):
        try:
            return self.sim_matrix_.loc[target_item].drop(target_item).sort_values(ascending=False).head(max_recommendation)
